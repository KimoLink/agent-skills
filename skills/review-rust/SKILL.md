...
使用本 skill 时，必须把 `review-engineering` 的通用工程基线和本 skill 的 Rust 专项清单合并使用，最终只输出一次评审、一次结论和一份报告。本 skill 只补充 Rust 专项要求，不能跳过或降低通用工程基线。

## 评审原则

- 使用中文输出。
- 不写模板腔、口号和对立转折式套话。
- 先确认评审依据层级：`review-engineering` 的通用硬性工程基线、本 skill 的 Rust 专项基线、用户或部门通用要求、项目已有规范、仓库事实、工具配置或团队约定。报告里要写清楚依据来自哪里。
...
- `AGENTS.md`、README、开发文档、架构文档、发布文档。
...
- README、AGENTS、架构文档、API 文档、部署文档、CLI help 是否与当前代码一致。
