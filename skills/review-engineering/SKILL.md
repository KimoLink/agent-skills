...
- 项目规则：存在的 `AGENTS.md`、README、开发文档、架构文档。
...
`AGENTS.md` 不是工程必需文件。仓库没有 `AGENTS.md` 时，不得把缺失本身写成问题；只有当仓库实际存在该文件，或用户、部门规范、项目文档明确要求它存在时，才检查其内容是否与当前工程事实一致。
...
- Node.js 项目的包安装环境应由个人环境、团队全局配置或 CI 运行环境配置处理。项目级 `.npmrc`、`bunfig.toml` 不应提交包源地址、scope 源、代理、认证、证书路径、registry 映射或其他会把某一内网、外网、个人电脑或 CI 环境强加给所有开发者的配置。团队私有 registry 也不应写入普通项目仓库；确需使用时，应通过全局配置、私有环境配置、CI secret 或明确隔离的本地说明处理。
...
- 如果同日期已有报告，必须优先合并到当天已有报告中，避免同一天多个零散报告。合并不是追加一套新的段落结构，也不是新增“本次复评”“第二次评审”“追加检查”之类未定义章节；必须把新的评审范围、检查情况、主要问题和整改建议融入既有同名章节，使文档读起来仍像一份完整报告。
- 合并同日报告时，保留仍然成立的原结论和证据；对已经变化的结论，在原章节中改写为当前事实，不要把旧结论和新结论并排堆叠。新增问题放入既有“主要问题”结构；新增命令结果放入既有“检查情况”结构；新增优先级建议放入既有“整改优先级建议”结构。
