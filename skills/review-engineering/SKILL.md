...
1. 先识别项目类型、技术栈和仓库边界。
2. 读取项目规范、构建入口、工具配置、仓库治理配置和已有文档，确认评审依据。
3. 如项目命中专项 skill，先采用本 skill 的通用基线，再合并专项清单；最终只输出一次评审、一次结论和一份报告。
4. 按项目事实选择可执行的只读检查命令；无法执行时说明原因，不把未验证状态写成通过。
5. 将结论落到 `docs/reviews/` 下的 Markdown 报告，并在最终回复中给出报告路径。
...
本 skill 是所有工程级评审的通用基线。Nuxt/Vue、Nest/Node、Unity、Unreal、Qt、.NET、Rust 等项目使用专项 skill 时，必须先阅读并吸收本 skill 的硬性工程基线，再读取 `stacks/` 下对应专项清单，把通用基线和专项清单合并成一次评审、一次结论和一份报告。

专项 skill 只是薄入口，负责触发和路由；实际专项检查项统一维护在 `review-engineering/stacks/*.md`。专项清单可以补充更细的技术栈检查项，不能跳过本 skill 的通用要求，不能把专项偏好写成抵消通用硬性基线的理由。

选择规则：

- Nuxt/Vue/Web 前端项目：读取 `stacks/nuxt.md`，结合通用工程基线做一次评审。
- NestJS/Node.js 后端项目：读取 `stacks/nest.md`，结合通用工程基线做一次评审。
- Unity 项目：读取 `stacks/unity.md`，结合通用工程基线做一次评审。
- Unreal Engine/UE/UE5 项目：读取 `stacks/unreal.md`，结合通用工程基线做一次评审。
- Qt Widgets/QML/C++ 桌面端项目：读取 `stacks/qt.md`，结合通用工程基线做一次评审。
- C#/.NET/ASP.NET Core/WPF/WinUI/Avalonia/MAUI 项目：读取 `stacks/dotnet.md`，结合通用工程基线做一次评审。
- Rust 后端/CLI/库/workspace 项目：读取 `stacks/rust.md`，结合通用工程基线做一次评审。
...
- 只读评审阶段擅自执行会修改仓库的命令。格式化、lint、fix、生成代码或生成资源等命令只有在用户明确转入修复时才能执行；评审检查优先使用 check、verify、dry-run 或只读等价命令。
...
格式化配置属于工程质量门禁。评审时优先执行 check、verify、dry-run 或只读等价命令；不要为了评审擅自格式化全仓，也不要执行会自动修改仓库的 lint、fix、生成代码或生成资源命令。
