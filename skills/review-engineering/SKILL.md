...
本 skill 是所有工程级评审的前置通用基线。Nuxt/Vue、Nest/Node、Unity、Unreal、Qt、.NET、Rust 等项目即使使用专项 skill，也必须先按本 skill 完成通用工程评审，再进入专项检查。
...
- Unreal Engine/UE/UE5 项目：先执行本 skill 的通用工程评审，再执行 `review-unreal` 专项评审。
- Qt Widgets/QML/C++ 桌面端项目：先执行本 skill 的通用工程评审，再执行 `review-qt` 专项评审。
- C#/.NET/ASP.NET Core/WPF/WinUI/Avalonia/MAUI 项目：先执行本 skill 的通用工程评审，再执行 `review-dotnet` 专项评审。
- Rust 后端/CLI/库/workspace 项目：先执行本 skill 的通用工程评审，再执行 `review-rust` 专项评审。
