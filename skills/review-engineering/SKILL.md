...
- 先确认评审依据层级：本 skill 的硬性工程基线、用户或部门通用要求、项目已有规范、仓库事实、工具配置、CI 或团队约定。报告里要写清楚依据来自哪里。
...
本 skill 是所有工程级评审的前置通用基线。Nuxt/Vue、Nest/Node、Unity 等项目即使使用专项 skill，也必须先按本 skill 完成通用工程评审，再进入专项检查。

专项 skill 可以补充更细的技术栈检查项，不能跳过本 skill 的通用要求，不能把专项偏好写成抵消通用硬性基线的理由。

选择规则：

- Nuxt/Vue/Web 前端项目：先执行本 skill 的通用工程评审，再执行 `review-nuxt` 专项评审。
- NestJS/Node.js 后端项目：先执行本 skill 的通用工程评审，再执行 `review-nest` 专项评审。
- Unity 项目：先执行本 skill 的通用工程评审，再执行 `review-unity` 专项评审。
- 混合仓库：先用本 skill 分出子系统和通用仓库风险，再对子系统套用对应专项 skill。
...
- 仓库配置：`.gitignore`、`.gitattributes`、Git LFS 规则、`.editorconfig`、CI 配置、提交规范文档。
...
包源选择本身不是问题。外网、内网、CI 可以使用不同包源；问题是仓库不应把某个环境的包源、认证、代理或本机路径强加给所有人。
按工具机制允许写入项目配置的包源信息不按普通本地环境配置处理，例如 Unity 的 scoped registry 可以写在 `Packages/manifest.json`。此类配置的评审重点是是否项目依赖需要、团队和 CI 是否可访问、scope 是否准确、是否夹带个人 token、临时账号或本机路径。
...
根据项目类型选择实际命令。原则是检查而不是擅自大范围修复。不是所有项目都适合在评审环境里自动构建，例如 Unity、Unreal、部分桌面端、嵌入式或依赖专用设备/授权/大资源的项目。遇到这种情况，不要强行构建；应说明阻断原因，并重点检查目录、资源、配置、命名、测试入口、日志产物、CI 或团队既有构建说明。
...
- 如有 CI 或 batchmode 命令，再检查 EditMode/PlayMode tests、编译日志和资源导入警告；没有自动化入口时不要硬上本地构建。
...
列出本次审查依据，包括本 skill 的硬性工程基线、用户或部门通用要求、项目规范、仓库配置、工具配置、CI 或团队约定。没有明确依据的判断，不写成违反规范；项目规范与硬性工程基线冲突时，应作为冲突风险记录。
