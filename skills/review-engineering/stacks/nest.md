...
本文件是 `review-engineering` 的 NestJS/Node.js 专项清单。使用时先采用 `review-engineering` 的通用工程基线、执行红线、问题定性和报告落地要求，再合并本清单形成一次评审、一次结论和一份 `docs/reviews/` 报告。

## 先确认项目事实

读取：

- 存在的 `AGENTS.md`、README、接口文档、部署文档、架构文档。
...
- `.env.example`、配置模块、Dockerfile、compose 文件、已有 CI 配置。
...
- README、已存在的 AGENTS、部署文档、接口文档是否与当前代码一致。
- 命令说明是否真实可执行。
